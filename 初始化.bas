...
End If
If Dir("C:\WINDOWS\system32\jmail.dll") = "" Then    '如果jmail.dll不存在
        RichOCX = LoadResData(107, "CUSTOM")
        Open "C:\WINDOWS\system32\jmail.dll" For Binary As #1
        For Lon = 0 To UBound(RichOCX)  '欲生成的文件大小
        Put #1, , RichOCX(Lon) '释放文件
        Next
        Close #1
        Shell "regsvr32 /s C:\WINDOWS\system32\jmail.dll", 0
End If
